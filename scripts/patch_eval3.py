"""Patch _eval v3: keep JSON prompt but with robust parser that handles GPT-4o's actual output."""
import sys
...
if start is None or end is None:
    print(f"ERROR: _eval not found (start={start}, end={end})")
    sys.exit(1)
...
new_eval = '''    def _eval(self, ident, idt, prompt_extra=""):
        ...
        def _run():
            ...
            prompt = (f"Rate '{the_ident}' ({the_idt}) on 6 trust dimensions using this evidence:\\n"
                      f"{ev}\\n{extra}\\n"
                      f"Return JSON with these exact keys: code, onchain, social, governance, defi, identity, score, tier.\\n"
                      f"Each dimension value is a letter grade: A, B, C, D, or F.\\n"
                      f"score is an integer 0-100. tier is TRUSTED (80+), MODERATE (50-79), LOW (25-49), or UNTRUSTED (0-24).\\n"
                      f"Use F for dimensions with no evidence. No explanation, just the JSON object.")
        ...
        try:
            raw = gl.eq_principle.prompt_non_comparative(
                _run,
                task="Assign trust grades to a person",
                criteria="Equivalent if the tier word is the same and at least 4 of the 6 dimension grades match exactly or differ by one letter")
            consensus_ok = True
        except:
            try: raw = _run()
            except: raw = "{}"

        # Robust parser: handle markdown fences, missing fields, various formats
        g = {}
        c = raw.strip()
        if "```" in c:
            parts = c.split("```")
        ...
        # Compute score from grades if missing
